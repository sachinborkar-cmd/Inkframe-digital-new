...
router.get('/', async (request, response, next) => {
  try {
    ...
    const [orders] = await pool.execute(
      `select o.id, e.title, o.amount_paise, o.status, o.payment_method, o.created_at
    ...
    response.json({ orders, books: uniqueBooks.map(book => ({...book, pdf_path:'/api/library/'+encodeURIComponent(book.slug)+'/download', epub_path:null})) });
  } catch (error) {
    next(error);
  }
});
