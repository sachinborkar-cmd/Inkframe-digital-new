...
(async()=>{
 try{
  ...
  assert.equal((await request('/api/admin/products/'+bookIds[0]+'/permanent',null,'DELETE')).status,409,'Order history prevents permanent deletion');
  const disposable=await request('/api/admin/products',{...product,title:'Delete test '+key,slug:'delete-'+key},'POST');
  assert.equal(disposable.status,200);bookIds.push(disposable.data.id);
  const deletionPath='/api/admin/products/'+disposable.data.id+'/permanent';
  assert.equal((await request(deletionPath,null,'DELETE','guest')).status,401);
  assert.equal((await request(deletionPath,null,'DELETE','owner',false)).status,403);
  await pool.execute('insert into carts(user_id,ebook_id) values (?,?)',[sessions.owner.userId,disposable.data.id]);
  await pool.execute('update coupons set ebook_id=? where id=?',[disposable.data.id,coupon.data.id]);
  assert.equal((await request(deletionPath,null,'DELETE')).status,409,'Linked coupon prevents deletion');
  await pool.execute('update coupons set ebook_id=? where id=?',[bookIds[0],coupon.data.id]);
  assert.equal((await request(deletionPath,null,'DELETE')).status,200);
  assert.equal((await request(deletionPath,null,'DELETE')).status,404);
  assert.ok(!(await request('/api/admin/products')).data.products.some(p=>p.id===disposable.data.id));
  const [[cartCount]]=await pool.execute('select count(*) total from carts where ebook_id=?',[disposable.data.id]);assert.equal(cartCount.total,0);
  const categoryPath='/api/admin/categories/'+cat.data.id;
  assert.equal((await request(categoryPath,null,'DELETE','guest')).status,401);
  assert.equal((await request(categoryPath,null,'DELETE','owner',false)).status,403);
  assert.equal((await request(categoryPath,null,'DELETE')).status,200);
  assert.equal((await request(categoryPath,null,'DELETE')).status,404);
  const remaining=(await request('/api/admin/products')).data.products.filter(p=>bookIds.includes(p.id));
  assert.equal(remaining.length,2);assert.ok(remaining.every(p=>p.category_id===null));
