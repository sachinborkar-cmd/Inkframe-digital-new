...
function selectedRows(){const q=($('#search')?.value||'').toLowerCase(),status=$('#status')?.value,day=$('#order-date')?.value;return data.filter(r=>(!q||(JSON.stringify(r).toLowerCase().includes(q)||(view==='orders'&&('#'+r.order_number).includes(q))))&&(!status||r.status===status)&&(!day||new Date(r.created_at).toLocaleDateString('en-CA')===day));}
