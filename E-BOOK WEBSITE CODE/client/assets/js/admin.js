...
function filters(statuses=[]){return '<div class="list-filters"><div class="search-control">'+icon('search')+'<input class="field" id="search" placeholder="Search '+escape(view)+'..." aria-label="Search '+escape(view)+'"></div>'+(statuses.length?'<select class="field !w-auto" id="status" aria-label="Filter status"><option value="">All statuses</option>'+statuses.map(s=>'<option>'+s+'</option>').join('')+'</select>':'')+(view==='orders'?'<input type="date" id="order-date" class="field !w-auto" aria-label="Filter order date">':'')+'<span class="filter-count" id="filter-count"></span></div><div id="list"></div>';}
function selectedRows(){const q=($('#search')?.value||'').toLowerCase(),status=$('#status')?.value,day=$('#order-date')?.value;return data.filter(r=>(!q||JSON.stringify(r).toLowerCase().includes(q))&&(!status||r.status===status)&&(!day||new Date(r.created_at).toLocaleDateString('en-CA')===day));}
function renderList(){const all=selectedRows();const pages=Math.max(1,Math.ceil(all.length/pageSize));page=Math.min(page,pages);const rows=all.slice((page-1)*pageSize,page*pageSize);let html='';
 ...
 if(view==='orders')html=table(['Order','Customer','Product','Payment','Total','Delivery','Downloads','Actions'],rows.map(r=>['#'+r.id+'<br>'+escape(date(r.created_at)),person(r.full_name,r.email),escape(r.title),badge(r.status)+'<br>'+escape(r.payment_method||'Unconfirmed'),money(r.amount_paise),r.email_sent_at?'Sent '+escape(date(r.email_sent_at)):'Not confirmed',escape(r.download_count),button('Details','details',r.id)+(r.status==='paid'?button('Resend PDF','resend',r.id):'')+(r.status==='paid'&&r.payment_method==='test'?button('Refund test order','refund',r.id):'')]));
...
document.addEventListener('submit',async e=>{
 ...
 try{const body=Object.fromEntries(new FormData(f));if(f.dataset.form==='products'){
  body.preview_pages=Array.from(f.querySelectorAll('[data-preview-row]')).map(r=>({path:r.querySelector('[data-preview-path]').value,caption:r.querySelector('[data-preview-caption]').value}));
...
document.addEventListener('click',async e=>{
 ...
 if(e.target.closest('[data-add-preview]')){if($('#preview-rows').children.length<12)$('#preview-rows').insertAdjacentHTML('beforeend',previewRow());else $('#dialog-message').textContent='You can add up to 12 preview pages.';return;}
