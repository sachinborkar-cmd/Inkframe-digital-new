...
function fileName(value){return (String(value||'').split('/').pop()||'No file uploaded').replace(/^[a-f\d-]{36}-/i,'');}
function setUploadInfo(input,path,name,preview=false){const container=preview?input.closest('[data-preview-row]'):input.parentElement;if(!container)return;let info=container.querySelector('[data-upload-info]');if(!info){info=document.createElement('p');info.className='upload-file-info';info.dataset.uploadInfo='';container.appendChild(info);}const label=name||fileName(path);info.textContent='File: '+label;if(typeof path==='string'&&path.startsWith('/')){const link=document.createElement('a');link.href=path;link.target='_blank';link.rel='noopener';link.textContent='View';link.className='upload-file-view';info.append(' ',link);}}
...
function renderList(){const all=selectedRows();const pages=Math.max(1,Math.ceil(all.length/pageSize));page=Math.min(page,pages);const rows=all.slice((page-1)*pageSize,page*pageSize);let html='';
 if(view==='products')html=table(['Select','Product','Category','Price','Purchases','Status','Actions'],rows.map(r=>['<input type="checkbox" data-product-check value="'+r.id+'" aria-label="Select '+escape(r.title)+'">',escape(r.title),escape(r.category||'-'),money(r.price_paise),escape(r.sales),badge(r.status),'<a class="nav-link" href="/product/?slug='+encodeURIComponent(r.slug)+'" target="_blank" rel="noopener">View</a>'+button('Edit','edit',r.id)+button('Archive','archive',r.id)+button('Delete','delete',r.id)]));
...
async function editor(row={}){
 if(view==='products'){
  ...
  f.querySelectorAll('[data-upload]').forEach(upload=>setUploadInfo(upload,f.elements[upload.dataset.upload].value));
  f.querySelectorAll('[data-preview-upload]').forEach(upload=>setUploadInfo(upload,upload.closest('[data-preview-row]').querySelector('[data-preview-path]').value,'',true));
...
document.addEventListener('click',async e=>{
 ...
 const clear=e.target.closest('[data-clear-file]');if(clear){const form=clear.closest('form'),field=form.elements[clear.dataset.clearFile];field.value='';const upload=form.querySelector('[data-upload="'+clear.dataset.clearFile+'"]');setUploadInfo(upload,'');return;}
